...
class SDBHelper(object):
    ...
    def attributesToBinary(cls, attributes):
        ...
        if numChunks:
            serializedJob = b''.join(bytes(v, 'utf-8)' for k, v in chunks)
...
def uploadFromPath(localFilePath, partSize, bucket, fileID, headers):
    ...
    if file_size <= partSize:
        key = bucket.new_key(key_name=bytes(fileID, 'utf-8'))
        key.name = fileID
    ...
    for attempt in retry_s3():
        with attempt:
            key = bucket.get_key(bytes(fileID, 'utf-8'),
                                 headers=headers,
                                 version_id=version)
