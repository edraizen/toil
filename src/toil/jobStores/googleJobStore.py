...
class GoogleJobStore(AbstractJobStore):
    ...
    def initialize(self, config=None):
        ...
        if self.config.sseKey is not None:
            with open(self.config.sseKey) as f:
                self.sseKey = bytes(f.read(), 'utf-8')
    ...
    def exists(self, jobStoreID):
        return self.bucket.blob(bytes(jobStoreID, 'utf-8'), encryption_key=self.sseKey).exists()

    @googleRetry
    def getPublicUrl(self, fileName):
        blob = self.bucket.get_blob(bytes(fileName, 'utf-8'), encryption_key=self.sseKey)
        if blob is None:
    ...
    def delete(self, jobStoreID):
        ...
        for blob in self.bucket.list_blobs(prefix=bytes(jobStoreID, 'utf-8')):
    ...
    def readFile(self, jobStoreFileID, localFilePath, symlink=False):
        ...
        with open(localFilePath, 'w') as writeable:
            blob = self.bucket.get_blob(bytes(jobStoreFileID, 'utf-8'), encryption_key=self.sseKey)
    ...
    def fileExists(self, jobStoreFileID):
        return self.bucket.blob(bytes(jobStoreFileID, 'utf-8'), encryption_key=self.sseKey).exists()
    ...
    def _getBlobFromURL(cls, url, exists=False):
        ...
        blob = bucket.blob(bytes(fileName, 'utf-8'))
    ...
    def readStatsAndLogging(self, callback, readAll=False):
        ...
        while True:
            filesReadThisLoop = 0
            # prefix seems broken
            for blob in self.bucket.list_blobs(prefix=bytes(prefix, 'utf-8')):
                try:
                    ...
                    if not readAll:
                        ...
                        self.bucket.rename_blob(blob, bytes(newID, 'utf-8'))
                except NoSuchFileException:
    ...
    def _delete(self, jobStoreFileID):
        if self.fileExists(jobStoreFileID):
            self.bucket.get_blob(bytes(jobStoreFileID, 'utf-8')).delete()
    ...
    def _readContents(self, jobStoreID):
        ...
        :rtype: string
        """
        job = self.bucket.get_blob(bytes(jobStoreID, 'utf-8'), encryption_key=self.sseKey)
        if job is None:
            raise NoSuchJobException(jobStoreID)
    ...
    def _writeFile(self, jobStoreID, fileObj, update=False, encrypt=True):
        blob = self.bucket.blob(bytes(jobStoreID, 'utf-8'), encryption_key=self.sseKey if encrypt else None)
        if not update:
    ...
    def _uploadStream(self, fileName, update=False, encrypt=True):
        ...
        """
        blob = self.bucket.blob(bytes(fileName, 'utf-8'), encryption_key=self.sseKey if encrypt else None)
    ...
    def _downloadStream(self, fileName, encrypt=True):
        ...
        """
        blob = self.bucket.get_blob(bytes(fileName, 'utf-8'), encryption_key=self.sseKey if encrypt else None)
        if blob is None:
            raise NoSuchFileException(fileName)
